...
from ScoutDrone.config import ModName
...
class ScoutDroneSettings(ScreenNode):
    ...
    def OnClickMoveButton(self, args):
        self.Display(False)
        self.client.SwitchState("edit_button")
    ...
    def Display(self, show):
        ...
        if show:
            if not self.mIsInitialized:
                self.client.CallServer("InitializeUI", PID)
            else:
                # 刷新一下数据
                ...
                for key, value in self.client.settings.items():
                    settingPath = scrollingContent + "/" + key
                    if not self.GetBaseUIControl(settingPath):
                        continue
    ...
    def OnClickPermissionButton2(self, args):
        ...
        self.client.CallServer("LoadDataForOthers")
    ...
    def InitializeUI(self, isOwner, defaultSettings):
        ...
        if not isOwner:
            ...
            def TryToOpenPermissionPanel(args):
                clientApi.GetSystem(ModName, "ClientSystem").CallServer("OpenPermissionPanel", PID)
    ...
    def SetData(self, key, value, isWorldData=False):
        ...
        self.client.CallServer("SetData", levelId if isWorldData else PID, key, value)
