...
from HealGun.config import ModName
...
class HealGunFunctions(ScreenNode):
        ...
        self.func_def = {
            ...
            "switch_bullet": {"name": "种类:{state}", "type": "switch",
                              "candidates": [("heal", {"name": "§d治疗弹"}),
                                             ...
                                             (
                                                 "poison",
                                                 {"name": "§2毒液弹", "settings_key": "poison_bullet_enabled"})]},
            "switch_tracking": {"name": "自动追踪:{state}", "type": "switch",
                                "candidates": [("yes", {"name": "§a开"}),
                                               ("no", {"name": "§c关"})]}
        }
    ...
    def Update(self):
        ...
        for func_key, func_def in self.func_def.items():
            ...
            if func_def['type'] == 'shoot':
                ...
                label_ctrl.SetText(
                    func_def['name'].format(rem=(bullet_capacity - self.bullet_rec[func_key]), max=bullet_capacity))
            elif func_def['type'] == 'skill':
                leftTime = self.use_cd[func_key] - time.time()
                if leftTime > 0.2:
                    label_ctrl.SetText(func_def['name'].format(color="e", cd="({}s)".format(round(leftTime, 1))))
                else:
        ...
        if show:
            if not self.initialized:
                ...
                for func_key, func_def in self.func_def.items():
                    ...
                    if func_def['type'] == 'shoot':
                        label_ctrl.SetText(
                            func_def['name'].format(rem=self.GetData("func_{}_capacity".format(func_key)),
    ...
    def on_click_up(self, args):
        ...
        if self.dragging_mode:
            ...
            if func_def['type'] == 'skill':
                ...
                if self.bullet_rec[self.clicking_func] >= bullet_capacity:
                    return
                ...
                self.bullet_rec[self.clicking_func] += 1
                self.func_label_ctrls[self.clicking_func].SetText(
                    func_def['name'].format(
                        ...
                        max=bullet_capacity))
                self.client.Shoot(self.state_rec['switch_bullet'])
            elif func_def['type'] == 'switch':
                now_i = -1
