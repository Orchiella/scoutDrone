...
from GravitationGun.config import ModName
...
class GravitationGunFunctions(ScreenNode):
        ...
        self.func_def = {
            ...
            "switch_force": {"name": "方向:{state}", "type": "switch",
                             "candidates": [("gravitation", {"name": "§b引力"}),
                                            ("repulsion", {"name": "§d斥力"})]},
            "switch_target": {"name": "目标:{state}", "type": "switch",
                              "candidates": [("entity", {"name": "§a实体"}),
                                             ("block", {"name": "§6方块"})]}
        }
    ...
    def Update(self):
        ...
        for func_key, func_def in self.func_def.items():
            ...
            if func_def['type'] == 'shoot':
                ...
                label_ctrl.SetText(
                    func_def['name'].format(color="f" if self.lock else "8",
